...
if (!function_exists('peakrackRiskIsWhitelisted')) {
    function peakrackRiskIsWhitelisted(object $client, string $emailDomain, string $ip, array $config): bool
    {
...
if (!function_exists('peakrackRiskScoreOrder')) {
    function peakrackRiskScoreOrder(object $order, object $client, array $vars, array $config): array
    {
        ...
        if (peakrackRiskIsWhitelisted($client, $emailDomain, $ip, $config)) {
            return [
                'score' => 0.0,
                'reasons' => ['Whitelisted'],
                'ip' => $ip,
        ...
        $providerScore = peakrackRiskFindScore((array) ($vars['fraudresults'] ?? []));
        ...
        if ($ip !== '') {
            ...
            if ($burstCount >= (int) $config['ipBurstOrderCount']) {
        ...
        $fraudHistory = Capsule::table('tblorders')
        ...
        if ($fraudHistory > 0) {
            $add = min(((int) $fraudHistory) * (float) $weights['historyFraudPerOrder'], (float) $weights['historyFraudMax']);
            $risk += $add;
        ...
        if ($activeServices > 0) {
            ...
            $risk += $add;
            $reasons[] = "ActiveService({$activeServices}){$add}";
        }
